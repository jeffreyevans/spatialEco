...
#' @description Modified Soil-adjusted Vegetation Index (MSAVI) or Modified Triangular Vegetation 
#'              Index 2 (MTVI) weighted by the Normalized difference senescent vegetation index (NDSVI)
#'
...
#'
#' @note
#' The intent of this index is to correct the MSAVI or MTVI index for bias associated with senescent vegetation. 
#' This is done by: 
...
#' The MSAVI index reduces soil noise and increases the dynamic range of the vegetation signal. 
#' The implemented modified version (MSAVI2) is based on an inductive method that does not use a 
#' constant L value, in separating soil effects, an highlights healthy vegetation.
#' The MTVI(2) index follows Haboudane et al., (2004) and represents the area of a hypothetical 
#' triangle in spectral space that connects (1) green peak reflectance, (2) minimum chlorophyll 
#' absorption, and (3) the NIR shoulder. When chlorophyll absorption causes a decrease of red 
#' reflectance, and leaf tissue abundance causes an increase in NIR reflectance, the total area of 
#' the triangle increases. It is good for estimating green LAI, but its sensitivity to chlorophyll 
#' increases with an increase in canopy density. The modified version of the index accounts for the 
#' background signature of soils while preserving sensitivity to LAI  and resistance to the influence of chlorophyll. 
#'
...
#' The threshold argument is used to apply a threshold to MSAVI. The default is NULL but if specified all 
#'   values [MSAVI <= threshold] will be NA.   
#' Applying a weight.factor can be used to change the influence of the weights on MSAVI. 
#' 
#' @references Haboudane, D., et al. (2004) Hyperspectral Vegetation Indices and Novel Algorithms for Predicting Green LAI 
#'             of Crop Canopies: Modeling and Validation in the Context of Precision Agriculture. 
#'             Remote Sensing of Environment 90:337-352.
#' @references Qi J., Chehbouni A., Huete A.R., Kerr Y.H., (1994). Modified Soil Adjusted Vegetation Index (MSAVI). 
#'               Remote Sens Environ 48:119-126.
#' @references Qi J., Kerr Y., Chehbouni A., (1994). External factor consideration in vegetation index development. Proc. of 
#'               Physical Measurements and Signatures in Remote Sensing, ISPRS, 723-730.
#' @references Qi, J., Marsett, R., Moran, M.S., Goodrich, D.C., Heilman, P., Kerr, Y.H., Dedieu, G., Chehbouni, A., Zhang, X.X. (2000).
#'               Spatial and temporal dynamics of vegetation in the San Pedro River basin area. 
#'               Agricultural and Forest Meteorology. 105:55-68. 
#'
