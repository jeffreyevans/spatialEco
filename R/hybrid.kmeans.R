...
#' @description Hybrid K-means clustering using hierarchical clustering to define cluster-centers 
#'
#' @param x        A data.frame or matrix with data to be clustered
#' @param k        Number of clusters
#' @param hmethod  The agglomeration method used in hclust, options are: "ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid"
...
#'
#' @return returns an object of class "kmeans" which has a print and a fitted method
#'
#' @details This method uses hierarchical clustering to define the cluster-centers in the K-means clustering algorithm. This mitigates some of the know convergence issues in K-means.   
#' 
...
hybrid.kmeans <- function(x, k = 2, hmethod = "ward.D", stat = mean, ...) {
 h.clust <- stats::hclust(dist(x), method=hmethod)
   cut.clust <- stats::cutree(h.clust, k = k) 
   cluster.centers <- aggregate(x, list(cut.clust), stat)[, -1]
